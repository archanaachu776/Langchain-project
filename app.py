...
st.markdown('<div class="sub-header">Your intelligent partner for deep research and quick answers.</div>', unsafe_allow_html=True)
    ...
    if not os.getenv("GROQ_API_KEY") or not os.getenv("TAVILY_API_KEY"):
        ...
        with st.spinner("🤖 Assistant is working..."):
            try:
                ...
                # Display Query
                st.markdown(f'<div class="query-display"><strong>Query:</strong> {result["query"]}</div>', unsafe_allow_html=True)
                st.write("")
                ...
                st.markdown("### 💡 Answer")
                st.markdown(f'<div class="success-box">{result["final_answer"]}</div>', unsafe_allow_html=True)
